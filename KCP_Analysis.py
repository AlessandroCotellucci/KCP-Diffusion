...
def Simulation(mu_HC,Time):
    #Preparing the graph
    #Num_HCW HCW nodes, each with 8 patient nodes as maximum number
    Num_HCW=10
    G=fast_gnp_random_graph(Num_HCW, 0.8, seed=None, directed=False)
    Role=['HCW']
    ...
    lambd=0.1      #Probability to add a patient
    ...
    for t in range(Time):
        ...
        if t>Time/2:
            ...
            alpha=0.19    #Antibiotic restriction decreasing alpha
...
Mu_HC=ones(100, 'double')
Time=1000
for r in range(100):
    Mu_HC[r]=r*0.01
    Num_infect[r]=Simulation(Mu_HC[r],Time)

#Plot of the exact and the numerical solution
plt.plot(Mu_HC,Num_infect,'r')
plt.xlabel(r'$\mu_{HC}$')
#plt.xlabel(r'$\alpha$')
plt.ylabel('Fraction of colonized nodes')
plt.title('KCP Diffusion')
plt.show()
